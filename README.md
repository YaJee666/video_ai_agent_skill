...
把仓库里的 `video-ai-agent` 文件夹复制到 Codex skills 目录：

```text
%USERPROFILE%\.codex\skills\video-ai-agent
```

然后启动新的 Codex 会话，让 Codex 重新发现 skill。

### 3. 安装到 Claude Code

把同一个 `video-ai-agent` 文件夹复制到 Claude Code 当前支持的 skills 目录，然后启动新的 Claude Code 会话。

这个 skill 是普通文件夹结构，包含 `SKILL.md`、`scripts/`、`references/` 和 `agents/`，适合支持 folder-based skills 的 agent 复用。
...
需要。开放接口使用 workspace API key 认证，最小 scope 是 `chat:write`。推荐使用 `X-API-Key` header，脚本会自动读取 `VIDEO_AI_AGENT_API_KEY`。
...
可以用 `--api-key`，但不推荐。更安全的方式是放到当前 shell 环境变量或你的本地 secrets 管理工具里，避免进入 shell history 或 Git。
