...
Video AI Agent Skill 为 Codex、Claude Code 和其他支持 Skill 的 coding agent 提供统一的视频与音频理解入口。只需给出媒体链接、本地音频文件和任务要求，agent 就可以调用线上 Video AI Agent 完成以下工作：

- **视频总结与要点提炼**：总结 Bilibili、YouTube、抖音、TikTok 和直接媒体 URL，输出摘要、核心观点、关键结论或行动清单。
- **字幕获取与转写整理**：优先使用可用字幕；没有字幕时由服务端尝试下载媒体并转写，随后整理口语、去除重复内容并生成结构化文本。
- **视频问答与内容检索**：围绕视频内容追问人物、观点、案例、时间点和结论，并在同一会话中继续深入。
- **多视频比较分析**：比较多个视频的观点、方法、论据和差异，适合竞品调研、课程对照、访谈归纳和资料综述。
- **本地音频上传与片段转写**：上传 `mp3`、`m4a`、`wav` 等音频，按指定时间范围裁剪并通过 Whisper 转写。
- **基于音视频内容写作**：将转写内容加工为文章、课程讲义、会议纪要、内容提纲、问答总结或社交媒体文案。
- **长任务与连续会话**：在线视频分析默认使用 jobs 模式处理长任务，并可通过 `sessionId` 保留上下文，继续追问或迭代输出。

本地只需要 Python 和 API key，不需要安装 GPU、Whisper、FFmpeg、yt-dlp，也不需要为不同视频平台分别维护下载和解析脚本。
